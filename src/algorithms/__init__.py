...
class BaseAgent(ABC):
        ...
        try:
            with open(config_path, "r") as file:
                ...
                if config is None or isinstance(config, str):
                    self.logger.warning(
                        f"Config file {config_path} contains no valid configuration. Using defaults."
                    )
                    ...
                    for key, value in default_config.items():
                        if key not in config:
                            config[key] = value
                            self.logger.info(
                                f"Added missing config parameter: {key} = {value}"
                            )
    ...
    @abstractmethod
    def act(
        self, state: Union[np.ndarray, torch.Tensor], **kwargs
        ...
        for key, values in metrics.items():
            if values:  # Only log if there are values
                if isinstance(values, list) and len(values) > 0:
                    ...
                    self.logger.info(
                        f"{key}: latest={latest_value:.4f}, avg_100={avg_value:.4f}"
                    )
                else:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"{self.algorithm_name}(state_size={self.state_size}, "
    ...
    **kwargs,
    ...
    try:
        if algorithm == "dqn":
            ...
            raise ValueError(
                f"Unknown algorithm: {algorithm}. Supported algorithms: dqn, ddpg, ppo, sac"
            )

    except NameError:
        raise ImportError(
            f"Algorithm '{algorithm}' is not available. Check if the implementation is properly imported."
        )
